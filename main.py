...
# --- 导演台 API ---
@app.post("/api/director/upload")
async def director_upload(file: UploadFile = File(...), type: str = "screenshot"):
    """
    接收导演台上传的截图或录屏文件，保存到 output/ 目录，
    并返回可供画布访问的 /output/ 路径。
    同时通过 WebSocket 通知所有连接的客户端。
    """
    import re as _re
    kind = str(type or "screenshot")
    ext = str(file.filename or "").rsplit(".", 1)[-1] if "." in str(file.filename or "") else ("png" if kind == "screenshot" else "mp4")
    safe_ext = _re.sub(r"[^a-zA-Z0-9]", "", ext)[:10] or ("png" if kind == "screenshot" else "mp4")
    ts = int(time.time())
    rand = uuid.uuid4().hex[:8]
    filename = f"director_{kind}_{ts}_{rand}.{safe_ext}"
    ...
    # 通过 WebSocket 通知所有客户端有新文件
    image_data = {
        "type": "director_output",
        "url": url,
        "kind": kind,
        "filename": filename,
        "created_at": ts,
    ...
    try:
        ...
        loop.create_task(manager.broadcast_new_image(image_data))
    except Exception:
        pass
    return {"ok": True, "url": url, "filename": filename, "kind": kind}


@app.get("/api/director/files")
def list_director_files(limit: int = 50):
    """列出导演台生成的文件，供画布侧加载。"""
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    files = []
    for fname in os.listdir(OUTPUT_DIR):
        if not fname.startswith("director_"):
            continue
    ...
    return {"files": files[:max(1, min(limit, 200))]}


# --- 导演台 WebSocket 通知支持 ---
# 前端 director.html 可连接 /ws/stats 复用同一 WebSocket，
# 后端已通过 broadcast_new_image 推送 director_output 消息。
